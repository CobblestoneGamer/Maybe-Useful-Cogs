...
class Osu:
    ...
    async def overview(self, ctx):
        ...
        if server_settings:
            if "api" not in server_settings:
                ...
                api = "Official osu! API"
            elif server_settings["api"] == self.osu_settings["type"]["yozora"]:
                api = "Yozora API"
        else:
            api = "Official osu! API"
    ...
    async def api(self, ctx, *, choice):
        ...
        if not choice.lower() == "official" and not choice.lower() == "yozora":
            await self.bot.say("The two choices are `official` and `yozora`")
            return
        elif choice.lower() == "official":
            db.osu_settings.update_one({'server_id':server.id}, {'$set':{"api": self.osu_settings["type"]["default"]}})
        elif choice.lower() == "yozora":
            db.osu_settings.update_one({'server_id':server.id}, {'$set':{"api": self.osu_settings["type"]["yozora"]}})
    ...
    def _determine_api(self, server, inputs):
        ...
        if not inputs or ('-yozora' not in inputs and '-official' not in inputs): # in case not specified
            server_settings = db.osu_settings.find_one({'server_id':server.id})
            if server_settings and "api" in server_settings:
                if server_settings["api"] == self.osu_settings["type"]["default"]:
                    ...
                    api = self.osu_settings["type"]["yozora"]
            else:
                api = self.osu_settings["type"]["default"]
        if '-yozora' in inputs:
            inputs = list(inputs)
            inputs.remove('-yozora')
            api = self.osu_settings["type"]["yozora"]
    ...
    def _get_discord_id(self, username:str, api:str):
        #if api == self.osu_settings["type"]["yozora"]:
            #name_type = "yozora_username"
        #else:
    ...
    async def _get_user_top(self, ctx, api, user, userbest, best_beatmaps, best_acc, gamemode, score_num = None, recent = False, web = False):
        ...
        elif api == self.osu_settings["type"]["yozora"]:
            profile_url = 'http://a.yozora.pw/{}.png'.format(user['user_id'])
    ...
    def _get_api_name(self, url:str):
        if url == self.osu_settings["type"]["yozora"]:
            return "yozora"
        else:
            return "Official"
